...
Static one-page site for La Pausa padel retreats. Plain HTML/CSS/JS — no build step.
...
fonts/Benniter.ttf                Brand display font (TRIAL license — license before commercial use)
.nojekyll                         Tells GitHub Pages to serve files as-is
```
Keep this folder structure intact — `index.html` references `assets/` and `fonts/` by relative path.

## Deploy with GitHub Pages
1. Create a new repository on GitHub (e.g. `lapausa-site`).
2. Upload the **contents of this folder** to the repo root (drag-and-drop on github.com → "uploading an existing file", or push with git).
...
- In **Settings → Pages → Custom domain**, enter `lapausapadel.com`.
